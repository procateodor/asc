...
const TOTAL_RISKS_SCORE = Object.values(RISKS).reduce(
  (sum, value) => sum + value,
  0
);
...
const getRiskLevel = (vulnerability) => {
  let risk = Object.keys(RISKS).reduce(
    (score, key) =>
      vulnerability.description[1].toLowerCase().includes(key)
        ? score + RISKS[key]
        : score,
    0
  );
...
//   `mysql://${process.env.USERNAME}:${process.env.PASSWORD}@${process.env.HOST}/${process.env.DATABASE}?reconnect=true`
// );
...
//       `insert into vulnerabilities (id, description, type, platform, date_published, verified, port, author, risk) values ('${
//         vul.id
//       }', '${vul.description[1]}', '${vul.type_id}', '${vul.platform_id}', '${
//         vul.date_published
//       }', '${vul.verified}', '${vul.port}', '${vul.author_id[1]}', '${Math.ceil(
//         getRiskLevel(vul)
//       )}')`
//     );
//   }
